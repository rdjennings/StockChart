...
const App = () => {
  ...
  useEffect(() => {
    const fetchTicker = () =>fetch('http://localhost:3010')
      ...
      .then(data => {
        ...
        let eod = new Date(year, month, day, 16, 0);
        ...
        if (weekDay < 1 || weekDay > 6) {
          if (weekDay === 0) {
            ...
            console.log(`%cToday is Sunday. Will retry on Monday, ${new Date(sod.getTime()).toString()} (${now})`, 'font-weight: bold; color: green');
          } else if (weekDay === 7) {
            const tomorrow = new Date(year, month, day + 2, 9, 30).getTime();
            setDelayState(true);
            delay = tomorrow - now.getTime();
            console.log(`%cToday is Saturday.Will retry on Monday, ${new Date(sod.getTime()).toString()} (${now})`, 'font-weight: bold; color: green');
          }
          ...
          setDelayState(true);
          delay = tomorrow - now.getTime();
          console.log(`%cWill retry on ${new Date(tomorrow).toString()} (${now})`, 'font-weight: bold; color: green');
        } else if (now.getTime() < sod.getTime()) {
          delay = sod.getTime() - now.getTime();
          console.log(`%cWill retry on ${new Date(sod.getTime()).toString()} (${now})`, 'font-weight: bold; color: green');
          setDelayState(true);
        } else {
          setDelayState(false);
        }
  ...
  const tickerBlocks = item => {
    ...
    if (delayState) {
      ...
      beep.play().catch((err) => 
        console.log(`%cOff hours tone did not play as user has not interacted with the page (${err.message})`, 'font-weight: bold; color: green')
      );
    } else {
      if (wasDelayed) {
        const beep = document.getElementById('beep');
        beep.play().catch((err) => 
          console.log(`%cOpen hours tone did not play as user has not interacted with the page (${err.message})`, 'font-weight: bold; color: green')
        );
      }
    }
